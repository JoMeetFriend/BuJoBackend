...
describe('createActivity - parseDateTime 過渡期同時接受舊格式與新格式（Backend accepts both legacy and new time string formats during the transition period）', () => {
  it('候選日期統一時間分別用舊格式（上午/下午）與新格式（HH:MM）送出，解析出相同的候選時段', async () => {
    ...
    await createActivity(
      makeReq({ body: { ...baseBody, uniformTime: { startTime: '上午 9:00', endTime: '上午 10:00' } } }),
      makeRes(),
    )
    ...
    prisma.activity.create.mockResolvedValue({ id: ACTIVITY_ID, candidateSlots: [] })
    await createActivity(
      makeReq({ body: { ...baseBody, uniformTime: { startTime: '09:00', endTime: '10:00' } } }),
      makeRes(),
    )
    const newFormatCall = prisma.activity.create.mock.calls.at(-1)[0]

    expect(newFormatCall.data.candidateSlots.create).toEqual(oldFormatCall.data.candidateSlots.create)
    expect(newFormatCall.data.schedule.create.deadline_at).toEqual(oldFormatCall.data.schedule.create.deadline_at)
  })
})
...
describe('listActivities - formatCard 候選時段顯示改用 formatHHMM，輸出零填充 24 小時制（Backend outputs displayed times in zero-padded 24-hour format）', () => {
  ...
  it('前後端一致性 fixture：第 9 小時的顯示字串跟前端 createTimeOptions()[9] 用同一個字面值 09:00', async () => {
    ...
    const [displayedStart] = res.json.mock.calls.at(-1)[0].activities[0].time.split(' - ')
...
describe('formatHHMM 輸出可以被 parseDateTime 新格式分支解析回相同小時分鐘（formatHHMM output round-trips through parseDateTime）', () => {
  it.each([0, 9, 12, 22])('候選時段第 %i 小時：透過 listActivities 顯示的字串，可以再送回 createActivity 解析出相同小時', async (hour) => {
    ...
    const [displayedStart, displayedEnd] = listRes.json.mock.calls.at(-1)[0].activities[0].time.split(' - ')
    ...
    expect(createCall.data.candidateSlots.create[0].slot_start.getHours()).toBe(hour)
  })
})
