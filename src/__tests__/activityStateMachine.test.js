...
jest.unstable_mockModule("../services/socketRoomService.js", () => ({
  syncOnActivityCreated: jest.fn(() => Promise.resolve()),
  syncOnActivityJoined: jest.fn(() => Promise.resolve()),
  syncOnActivityLeft: jest.fn(() => Promise.resolve()),
}));
...
describe("createActivity - 活動名稱長度驗證", () => {
  ...
  it("title 超過 15 字時回 400，且不建立活動", async () => {
    const req = makeReq({
      body: { ...validFixedBody, title: "1234567890123456" },
    });
    const res = makeRes();
  ...
  it("title trim 後剛好 15 字時可建立，activity title 與 chat name 使用 trim 後文字", async () => {
    ...
    const req = makeReq({
      body: { ...validFixedBody, title: "  123456789012345  " },
    });
    const res = makeRes();
...
describe("getActivity - recruiting 到期後的合法自動轉移", () => {
  ...
  it("尚未到期時維持 recruiting，不觸發任何狀態轉移", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({
        activity: expect.objectContaining({ status: "recruiting" }),
      }),
    );
  });
  ...
  it("情境三／四建立活動不再要求 creatorSlotIndexes，缺少這個欄位也能成功建立", async () => {
    ...
    const req = makeReq({
      body: {
        ...
        uniformTime: { startTime: "09:00", endTime: "10:00" },
  ...
  it("候選日期不連續時，deadline_at 等於最晚候選日的 slot_start，vote_deadline_at 等於送出的 deadline", async () => {
    prisma.activity.create.mockResolvedValue({
      ...
      candidateSlots: [
        ...
        {
          id: "slot-2",
          slot_start: new Date("2026-08-20T01:00:00Z"),
          slot_end: new Date("2026-08-20T02:00:00Z"),
        },
        {
          id: "slot-3",
          slot_start: new Date("2026-08-29T01:00:00Z"),
  ...
  it("候選日期各自時段、日期不連續時，deadline_at 等於最晚候選時段的 slot_start，vote_deadline_at 等於送出的 deadline", async () => {
    prisma.activity.create.mockResolvedValue({
      ...
      candidateSlots: [
        ...
        {
          id: "slot-2",
          slot_start: new Date("2026-08-20T11:00:00Z"),
          slot_end: new Date("2026-08-20T12:00:00Z"),
        },
        {
          id: "slot-3",
          slot_start: new Date("2026-08-10T01:00:00Z"),
          slot_end: new Date("2026-08-10T02:00:00Z"),
        },
      ],
    });
    ...
    const req = makeReq({
      body: {
        ...
        // 刻意打亂順序，確認不是「取陣列最後一筆」而是真的比較時間
        dateSlots: [
          { date: "2026/08/20", startTime: "19:00", endTime: "20:00" },
          { date: "2026/08/01", startTime: "09:00", endTime: "10:00" },
  ...
  it.each([
    ...
    [
      "情境四（候選日期各自時段）",
      {
        ...
        deadline: new Date(2020, 6, 1).toISOString(),
        dateSlots: [
          { date: "2020/08/01", startTime: "09:00", endTime: "10:00" },
          { date: "2020/08/02", startTime: "09:00", endTime: "10:00" },
        ],
      },
    ],
  ])(
    "%s：算出的 deadline_at 已經過去時回 400，且不建立任何紀錄",
    async (_label, body) => {
...
describe("createActivity - 結束時間必須晚於開始時間（前端有擋，但這是公開 API，後端要重新驗證）", () => {
  it("情境一：endTime 早於 startTime（同一天）時回 400，且不建立任何紀錄", async () => {
    const req = makeReq({
      body: {
        title: "結束時間早於開始時間-情境一",
  ...
  it("情境一（allDay）：endDate 早於 startDate 時回 400，且不建立任何紀錄", async () => {
    const req = makeReq({
      body: {
        title: "結束日期早於開始日期-情境一整日",
        deadline: new Date(2099, 0, 1).toISOString(),
  ...
  it("情境二：timeWindowEnd 早於 timeWindowStart 時回 400，且不建立任何紀錄", async () => {
    const req = makeReq({
      body: {
        title: "結束時間早於開始時間-情境二",
        deadline: new Date(2099, 0, 1).toISOString(),
        singleDate: "2099/01/02",
  ...
  it("情境三：uniformTime.endTime 早於 startTime 時回 400，且不建立任何紀錄", async () => {
    const req = makeReq({
      body: {
        title: "結束時間早於開始時間-情境三",
        deadline: new Date(2099, 0, 1).toISOString(),
        candidateDates: ["2099/01/02", "2099/01/03"],
        uniformTime: { startTime: "18:00", endTime: "09:00" },
      },
    });
    const res = makeRes();
  ...
  it("情境四：任一筆 dateSlots 的 endTime 早於自己的 startTime 時回 400，且不建立任何紀錄", async () => {
    const req = makeReq({
      body: {
        title: "結束時間早於開始時間-情境四",
        deadline: new Date(2099, 0, 1).toISOString(),
        dateSlots: [
          { date: "2099/01/02", startTime: "09:00", endTime: "10:00" },
          { date: "2099/01/03", startTime: "18:00", endTime: "09:00" },
        ],
      },
    });
    const res = makeRes();
...
describe("joinActivity - 只能報名 recruiting 中的活動", () => {
  it.each(["voting", "confirmed", "cancelled"])(
    "狀態為 %s 時拒絕報名",
    async (status) => {
...
describe("joinActivity - 報名後人數達標，通知建立者，但不自動成團（Reaching the participant target never auto-confirms an activity）", () => {
  ...
  it("投票制達標且全員一致時仍轉 voting、不自動成團，交由建立者手動確認", async () => {
    const slotA = makeSlot("slot-a", {
      availabilities: [{ candidate_slot_id: "slot-a", user_id: CREATOR_ID }],
    });
  ...
  it("建立者在收到通知後手動呼叫 confirmFormation 能正常成團", async () => {
    ...
    await confirmFormation(
      makeReq({ body: { candidateSlotId: "slot-a" } }),
      res,
    );
...
describe("joinActivity - Scenario C slot resubmission during recruiting", () => {
    ...
    return makeActivity({
      ...
      candidateSlots: [
        makeSlot("slot-a", {
          ...
          slot_end: new Date("2026-08-01T12:00:00Z"),
          availabilities: [
            { candidate_slot_id: "slot-a", user_id: PARTICIPANT_ID },
          ],
        }),
        makeSlot("slot-b", {
          slot_start: new Date("2026-08-02T10:00:00Z"),
  ...
  it("已報名者於 recruiting 重新送 candidateSlotIds 時覆寫 ActivityAvailability，不新增 participant", async () => {
    ...
    expect(prisma.activityAvailability.deleteMany).toHaveBeenCalledWith({
      where: {
        ...
        candidateSlot: { activity_id: ACTIVITY_ID },
      },
    });
  ...
  it.each(["voting", "confirmed"])(
    "已報名者於 %s 重新送 candidateSlotIds 時回 400 且不改資料",
    async (status) => {
      prisma.activity.findUnique.mockResolvedValue(
        makeScenarioCActivity({ status }),
      );
      prisma.activityParticipant.findUnique.mockResolvedValue({
        id: "participant-row-1",
        status: "joined",
          ...
          candidateSlotRanges: [
            {
              ...
              rangeStart: "2026-08-01T10:30:00Z",
              rangeEnd: "2026-08-01T11:30:00Z",
            },
          ],
        },
      }),
      ...
      makeReq({
        ...
        body: {
          ...
          candidateSlotRanges: [
            {
              candidateSlotId: "slot-a",
              rangeStart: "2026-08-01T09:00:00Z",
              rangeEnd: "2026-08-01T11:30:00Z",
            },
          ],
        },
...
describe("getActivity - Formation decision candidates are not filtered to only the leading option", () => {
  ...
  it("情境四：非建立者請求時 decision_candidates 為 null；建立者不受影響", async () => {
    ...
    expect(nonCreatorRes.json).toHaveBeenCalledWith(
      expect.objectContaining({
        activity: expect.objectContaining({ decision_candidates: null }),
      }),
    );
    ...
    expect(creatorRes.json).toHaveBeenCalledWith(
      expect.objectContaining({
        activity: expect.objectContaining({
          decision_candidates: expect.arrayContaining([
            expect.objectContaining({ id: "slot-x", count: 1 }),
          ]),
        }),
      }),
    );
  ...
  it("情境四：candidate_slots co_participants 用子區間交集運算篩出跟自己重疊的人，沒填子區間視為整個窗口，未選的候選時段是空陣列", async () => {
    ...
    const slotXResult = responseBody.activity.candidate_slots.find(
      (s) => s.id === "slot-x",
    );
...
describe("getActivity - candidate_slots 附上目前使用者自己的勾選狀態", () => {
  it("回傳的 candidate_slots 依目前使用者是否已在該時段留下 availability 標記 is_selected", async () => {
    ...
    const slotB = makeSlot("slot-b", {
      availabilities: [{ candidate_slot_id: "slot-b", user_id: CREATOR_ID }],
    });
    const activity = makeActivity({
      candidateSlots: [slotA, slotB],
      participants: [
...
describe("getActivity - 情境三（find_date）decision_candidates 限定建立者、candidate_slots 新增 co_participants", () => {
  function makeScenarioCActivity(overrides = {}) {
    ...
    const slotY = makeSlot("slot-y", {
      ...
      slot_end: new Date("2026-08-02T12:00:00Z"),
      availabilities: [
        { candidate_slot_id: "slot-y", user_id: "participant-3" },
      ],
    });
    return makeActivity({
      candidateSlots: [slotX, slotY],
  ...
  it("非建立者請求時 decision_candidates 為 null；建立者不受影響", async () => {
    ...
    expect(creatorRes.json).toHaveBeenCalledWith(
      expect.objectContaining({
        activity: expect.objectContaining({
          decision_candidates: expect.arrayContaining([
            expect.objectContaining({ id: "slot-x", count: 2 }),
          ]),
        }),
      }),
    );
...
describe("confirmFormation - voting 合法轉移到 confirmed", () => {
  ...
  it.each(["confirmed", "cancelled"])(
    ...
    async (status) => {
      ...
      await confirmFormation(
        makeReq({ body: { candidateSlotId: "slot-1" } }),
        res,
      );
  ...
  it("情境三可以確認非最高票的候選時段（建立者自由選，不限並列最高票）", async () => {
    ...
    await confirmFormation(
      makeReq({ body: { candidateSlotId: "slot-b" } }),
      res,
    );
  ...
  it("情境三選擇不屬於此活動的候選時段會被拒絕", async () => {
    ...
    await confirmFormation(
      makeReq({ body: { candidateSlotId: "slot-not-exist" } }),
      res,
    );

    expect(res.status).toHaveBeenCalledWith(400);
    expect(res.json).toHaveBeenCalledWith({
      message: "此候選時段不在可確認的名單中",
    });
    expect(prisma.activity.updateMany).not.toHaveBeenCalled();
  });
...
describe("createActivity - parseDateTime 過渡期同時接受舊格式與新格式（Backend accepts both legacy and new time string formats during the transition period）", () => {
  it("候選日期統一時間分別用舊格式（上午/下午）與新格式（HH:MM）送出，解析出相同的候選時段", async () => {
    const baseBody = {
      title: "雙格式測試",
      deadline: new Date("2026-07-31T00:00:00Z").toISOString(),
      candidateDates: ["2026/08/01"],
    ...
    const oldFormatCall = prisma.activity.create.mock.calls.at(-1)[0];
    ...
    const newFormatCall = prisma.activity.create.mock.calls.at(-1)[0];

    expect(newFormatCall.data.candidateSlots.create).toEqual(
      oldFormatCall.data.candidateSlots.create,
    );
    expect(newFormatCall.data.schedule.create.deadline_at).toEqual(
      oldFormatCall.data.schedule.create.deadline_at,
    );
  });
});

describe("listActivities - formatCard 候選時段顯示改用 formatHHMM，輸出零填充 24 小時制（Backend outputs displayed times in zero-padded 24-hour format）", () => {
  it("候選時段時間不再輸出上午/下午，改成零填充 24 小時制 HH:MM", async () => {
    const activity = makeActivity({
      status: "recruiting",
      candidateSlots: [
        makeSlot("slot-1", {
          slot_start: new Date(2026, 7, 1, 9, 0),
          slot_end: new Date(2026, 7, 1, 10, 0),
        }),
      ],
      schedule: {
        requires_voting: false,
    ...
    expect(res.json).toHaveBeenCalledWith({
      activities: [expect.objectContaining({ time: "09:00 - 10:00" })],
    });
  });

  it("候選時段時間跨到下午的小時，同樣輸出零填充 24 小時制（不是上午/下午 12 進位）", async () => {
    const activity = makeActivity({
      status: "recruiting",
      candidateSlots: [
        makeSlot("slot-1", {
          slot_start: new Date(2026, 7, 1, 23, 0),
          slot_end: new Date(2026, 7, 1, 23, 30),
        }),
      ],
      schedule: {
        requires_voting: false,
    ...
    expect(res.json).toHaveBeenCalledWith({
      activities: [expect.objectContaining({ time: "23:00 - 23:30" })],
    });
  });

  // 前後端一致性 fixture：這個字面值 '09:00' 要跟 BuJo repo 對應 change 裡
  // src/utils/timeFormat.js 的 createTimeOptions()[9] 斷言使用同一個字串，
  // 確保前端「第 9 個選項」跟後端「上午 9 點的顯示格式」是同一份 24 小時制格式定義
  it("前後端一致性 fixture：第 9 小時的顯示字串跟前端 createTimeOptions()[9] 用同一個字面值 09:00", async () => {
    const activity = makeActivity({
      status: "recruiting",
      candidateSlots: [
        makeSlot("slot-1", {
          slot_start: new Date(2026, 7, 1, 9, 0),
          slot_end: new Date(2026, 7, 1, 9, 30),
        }),
      ],
      schedule: {
        requires_voting: false,
    ...
    const [displayedStart] = res.json.mock.calls
      .at(-1)[0]
      .activities[0].time.split(" - ");
    expect(displayedStart).toBe("09:00");
  });
});

describe("formatHHMM 輸出可以被 parseDateTime 新格式分支解析回相同小時分鐘（formatHHMM output round-trips through parseDateTime）", () => {
  it.each([0, 9, 12, 22])(
    "候選時段第 %i 小時：透過 listActivities 顯示的字串，可以再送回 createActivity 解析出相同小時",
    async (hour) => {
      const displayActivity = makeActivity({
        status: "recruiting",
        candidateSlots: [
          makeSlot("slot-1", {
            slot_start: new Date(2026, 7, 1, hour, 0),
            slot_end: new Date(2026, 7, 1, hour + 1, 0),
          }),
        ],
        schedule: {
          requires_voting: false,
      ...
      prisma.activity.findMany.mockResolvedValue([displayActivity]);
      const listRes = makeRes();
      await listActivities(makeReq(), listRes);
      const [displayedStart, displayedEnd] = listRes.json.mock.calls
        .at(-1)[0]
        .activities[0].time.split(" - ");
      ...
      await createActivity(
        makeReq({
          body: {
            ...
            deadline: new Date("2026-07-31T00:00:00Z").toISOString(),
            candidateDates: ["2026/08/01"],
            uniformTime: { startTime: displayedStart, endTime: displayedEnd },
          },
        }),
        makeRes(),
      );
      const createCall = prisma.activity.create.mock.calls.at(-1)[0];

      expect(
        createCall.data.candidateSlots.create[0].slot_start.getHours(),
      ).toBe(hour);
    },
  );
});
...
describe("lazy 狀態轉換的 LINE 推播（LINE push delivery for activity lifecycle notifications）", () => {
  ...
  it("招募截止轉入 voting 後對建立者推播 time_to_pick 文案", async () => {
    ...
    prisma.userIdentity.findFirst.mockResolvedValue({
      provider_user_id: "U-line-creator",
    });
    const res = makeRes();
...
describe("joinActivity／confirmFormation／cancelActivity 的 LINE 推播接線", () => {
  ...
  it("確認成團後對其他參與者推播 activity_confirmed 文案，建立者自己不收推播", async () => {
    ...
    prisma.userIdentity.findFirst.mockResolvedValue({
      provider_user_id: "U-line-participant",
    });
    const res = makeRes();
