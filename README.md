...
BuJo 應用的後端 API，提供帳號認證、好友系統、活動揪團、站內通知／LINE 推播與地址自動完成功能。
...
- **Google OAuth / LINE Login** — 第三方登入，皆為後端 OAuth redirect 流程
- **LINE Messaging API** — 活動相關通知的 LINE 推播
- **Cloudinary** — 使用者頭像上傳
- **LocationIQ** — 地址自動完成（含記憶體快取）
- **Jest + Supertest** — 測試
...
| `GOOGLE_CLIENT_SECRET` | Google OAuth Client Secret，**絕不可外洩或提交到版控**           |
| `GOOGLE_CALLBACK_URL` | Google OAuth 完成後的 callback 網址（需與 Google Cloud Console 設定一致） |
| `FRONTEND_URL`       | 前端網址，第三方登入完成後導回前端使用                             |
...
| `LOCATIONIQ_API_KEY` | LocationIQ API key，地址自動完成（`/api/places/autocomplete`）使用    |
...
- **Google 登入 / 綁定**：`GET /api/auth/google` 建立 `user_id = null` 的一次性 OAuth attempt 並導向 Google 授權頁；已登入使用者由 `GET /api/auth/google/link` 以目前登入者 ID 建立 attempt 後導向。Google 導回 `GET /api/auth/google/callback` 後由後端以 `code` 交換 access token 並驗證 ID Token
- **LINE Login / 綁定**：`GET /api/auth/line` 使用 `bot_prompt=normal`；已登入使用者由 `GET /api/auth/line/link` 使用 `bot_prompt=aggressive`。LINE 導回 `GET /api/auth/line/callback`

Google 與 LINE 的 callback 都會先驗證並一次性消耗 `state`（存於 `OAuthAttempt` 表，雜湊儲存、10 分鐘過期），再由 attempt 的 `user_id` 判斷這次是 login 或 link。login 成功會簽發 `token` cookie 並回前端首頁；link 成功回 `/profile/edit?linked=google`（或 `line`）；link 取消或失敗分別回 `/profile/edit?error=..._link_cancelled`、`/profile/edit?error=..._link_failed`，不會把已登入使用者送回登入頁。整個 OAuth 流程（state 驗證、code 交換）完全在後端處理，三個 endpoint 一律是 302 redirect，不回傳 JSON
...
`DATABASE_URL`、`JWT_SECRET`、`GOOGLE_CLIENT_ID`、`GOOGLE_CLIENT_SECRET`、`GOOGLE_CALLBACK_URL`、`LINE_CHANNEL_ID`、`LINE_CHANNEL_SECRET`、`LINE_CALLBACK_URL`、`LINE_MESSAGING_CHANNEL_ACCESS_TOKEN`、`LINE_PUSH_ENABLED`、`ALLOWED_ORIGINS`、`FRONTEND_URL`、`CLOUDINARY_CLOUD_NAME`、`CLOUDINARY_API_KEY`、`CLOUDINARY_API_SECRET`、`CLOUDINARY_AVATAR_FOLDER`、`LOCATIONIQ_API_KEY`
